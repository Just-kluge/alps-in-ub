...
def build_same_rack_diff_col_paths(
    ...
    for layer in range(LRS_LAYERS):
        ...
        # 所有 layer 统一使用与前面 if 相同的 DU->ToR 路由规则（按 0/1 组复用映射）。
        layer_group = layer % 2
        port_index_base = 0 if src_l1%2 ==0 else 2
        for l1_dst_du_link_idx in (0, 1):
            # 根据 layer_group、目的行、l1_dst_du_link_idx 确定 DU->ToR 的中间 ToR 索引。
            dst_row = dst.rack_r
            mid_tor_row = portMap[layer_group]["cross_colunm"][port_index_base + l1_dst_du_link_idx]
    ...
    for layer in range(LRS_LAYERS):
        ...
        for dst_du_idx in (2 * dst.rack_c, 2 * dst.rack_c + 1):
            ...
            for hrs_dst_du_link_idx in range(4):
                mid_tor_row = portMap[layer_group]["cross_rack"][hrs_dst_du_link_idx]
...
def main() -> None:
    ...
    parser.add_argument(
        "--case-dir",
        default="/app/ns-3-ub/scratch/tp_UB_Mesh_V0003",#4月1号下午，jyxiao改
