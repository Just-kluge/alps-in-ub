...
def build_same_rack_diff_col_paths(
    ...
    for layer in range(LRS_LAYERS):
        ...
        # 简化规则：layer i 的 DU 仅连接到 ToR 行 2i 和 2i+1。
        tor_rows = [2 * layer, 2 * layer + 1]
        for l1_dst_du_link_idx in (0, 1):
            dst_row = dst.rack_r
            mid_tor_row = tor_rows[l1_dst_du_link_idx]
...
def build_same_pod_diff_rack_paths(
    ...
    for layer in range(LRS_LAYERS):
        ...
        # 简化规则：layer i 的 DU 中转仅通过 ToR 行 2i 和 2i+1。
        transit_tor_rows = [2 * layer, 2 * layer + 1]
        
        # 遍历目的列对应的两类 DU（偶数/奇数）。
        for dst_du_idx in (2 * dst.rack_c, 2 * dst.rack_c + 1):
            dst_du = du_id(dst.pod, dst.rack, layer, dst_du_idx)
            
            # HRS->目的 DU 的 4 条并行链路中，
            # 前两条按“直达目的 ToR”，后两条按“经 2i/2i+1 中转”。
            for hrs_dst_du_link_idx in range(4):
                dst_row = dst.rack_r  # 目标NPU所在的行序号
                is_direct = hrs_dst_du_link_idx in (0, 1)
                mid_tor_row = transit_tor_rows[hrs_dst_du_link_idx - 2] if not is_direct else dst_row

                # 构建节点序列。
                if is_direct or mid_tor_row == dst_row:
...
def main() -> None:
    ...
    parser.add_argument(
        "--case-dir",
        default="/app/ns-3-ub/scratch/tp_UB_Mesh_V0002",#4月22号下午，jyxiao改
