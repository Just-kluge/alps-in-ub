...
RACK_ROWS = 8
RACK_COLS = 8
RACK_HOSTS_PER_RACK = RACK_ROWS * RACK_COLS
RACK_TORS_PER_RACK = RACK_ROWS * RACK_COLS
...
if HRS_NUM != LRS_LAYERS:
    raise ValueError(
        "新拓扑要求 HRS_NUM == LRS_LAYERS（layer i 绑定 HRS[i]），"
        f"当前 HRS_NUM={HRS_NUM}, LRS_LAYERS={LRS_LAYERS}"
    )
...
# 解释：
# - 对于一个 8x8 Rack 内 ToR：
#   仅同列可连 (RACK_ROWS - 1) = 7 个 ToR
# - 再加 1 个下连 Host 端口 + 8 个上连 LRS 层端口
#   得到总端口数 16。
# - 这里将 Host<->ToR 带宽设为 (TOR_TOTAL_PORTS - 1) * BASE_BW_GBPS，
#   体现为接入侧聚合能力（不改变原有建模逻辑，仅解释设计意图）。

TOR_TOTAL_PORTS = 1 + (RACK_ROWS - 1) + (2 * LRS_LAYERS)

BW_HOST_TOR = f"{(TOR_TOTAL_PORTS - 1) * BASE_BW_GBPS}Gbps"
BW_TOR_TOR_IN_RACK = BW_BASE
BW_TOR_DU = BW_BASE
BW_DU_L1 = BW_BASE
BW_DU_HRS = BW_BASE
...
def build_topology():
    ...
    # 验证点：应生成 POD_NUM * RACK_NUM * RACK_ROWS * RACK_COLS 个 Host。

    for pod in range(POD_NUM):

        for rack in range(RACK_NUM):

            for rr in range(RACK_ROWS):
                for rc in range(RACK_COLS):

                    key = (pod, rack, rr, rc)

                    host_id_map[key] = next_id

                    graph.add_netisim_host(next_id, forward_delay="1ns")
        ...
        for rack in range(RACK_NUM):
            ...
            for rr in range(RACK_ROWS):
                for rc in range(RACK_COLS):
                    ...
                    graph.add_netisim_node(next_id, forward_delay="100ns")
    ...
    # LRS 分层设计说明：
    # - 每个 Rack 有 LRS_LAYERS 层，每层包含 DU_PER_LRS 个 DU 和 L1_PER_LRS 个 L1。
    # - 新拓扑中每层为 16 DU + 4 L1，DU 负责向上连接 HRS，L1 仅作为本层本 Rack 内部汇聚。
    # - layer 维度用于提供多条并行上行路径。
    # 验证点：
    # - DU 总数应为 POD_NUM * RACK_NUM * LRS_LAYERS * DU_PER_LRS
    # - L1 总数应为 POD_NUM * RACK_NUM * LRS_LAYERS * L1_PER_LRS
    ...
    for pod in range(POD_NUM):
        ...
        for rack in range(RACK_NUM):
            ...
            for layer in range(LRS_LAYERS):
                ...
                # DU
                for du_idx in range(DU_PER_LRS):

                    key = (pod, rack, layer, du_idx)
 ...
 # 第二步：生成所有 L1（在所有 DU 之后）
    for pod in range(POD_NUM):
        ...
        for rack in range(RACK_NUM):
            ...
            for layer in range(LRS_LAYERS):
                ...
                for l1_idx in range(L1_PER_LRS):

                    key = (pod, rack, layer, l1_idx)
    ...
    # 验证点：hrs_ids 长度应等于 HRS_NUM。

    for _ in range(HRS_NUM):

        hrs_ids.append(next_id)

        graph.add_netisim_node(next_id, forward_delay="250ns")
    ...
    for key, hid in host_id_map.items():
    ...
    # - 因此每个 ToR 的 Rack 内邻居数 = (RACK_ROWS - 1)。
    # - 在 8x8 时即 7 个邻居。
    #
    # 去重策略（避免重复计数）:
    ...
    #
    # 验证点（单 Rack）:
    # - 列内边数 = RACK_COLS * C(RACK_ROWS, 2)
    # - 总边数（全网） = POD_NUM * RACK_NUM * RACK_COLS * C(RACK_ROWS, 2)
        ...
        for rack in range(RACK_NUM):
            ...
            for rr in range(RACK_ROWS):
                for rc in range(RACK_COLS):
                    ...
                    src = tor_id_map[(pod, rack, rr, rc)]

                    # 同列
                    for rr2 in range(RACK_ROWS):

                        if rr2 == rr:
                            continue

                        dst = tor_id_map[(pod, rack, rr2, rc)]

                        # 同样使用 src < dst 去重，避免重复添加同一物理链路。
                        if src < dst:
                            ...
                            graph.add_netisim_edge(
                                src,
                                dst,
                                bandwidth=BW_TOR_TOR_IN_RACK,
                                delay=DELAY_TOR_TOR_IN_RACK,
                                edge_count=1,
                            )
    ...
    #
    # 验证点：
    # - 每个 (pod, rack, col, layer) 组合会生成 RACK_ROWS * 2 条 ToR->DU 边。
    # - 总边数 = POD_NUM * RACK_NUM * RACK_COLS * LRS_LAYERS * RACK_ROWS * 2。
    ...
    for pod in range(POD_NUM):
        ...
        for rack in range(RACK_NUM):
            ...
            for col in range(RACK_COLS):
                ...
                for layer in range(LRS_LAYERS):
                    du_even = du_id_map[(pod, rack, layer, 2 * col)]
                    du_odd = du_id_map[(pod, rack, layer, 2 * col + 1)]

                    for row in range(RACK_ROWS):

                        tor = tor_id_map[(pod, rack, row, col)]

                        # add_netisim_edge(tor, du_even, ...)
                        # - tor    : (pod, rack, row, col) 的 ToR 全局 ID
                        # - du_even: (pod, rack, layer, 2*col) 的 DU 全局 ID
                        graph.add_netisim_edge(
                            tor,
                            du_even,
                            bandwidth=BW_TOR_DU,
                        ...
                        # - tor: (pod, rack, row, col) 的 ToR 全局 ID
                        # - du_odd : (pod, rack, layer, 2*col+1) 的 DU 全局 ID
    ...
    # - 总调用次数 = POD_NUM * RACK_NUM * LRS_LAYERS * DU_PER_LRS * 2
    # - 物理链路总数 = 上式 * 2
        ...
        for rack in range(RACK_NUM):
            ...
            for layer in range(LRS_LAYERS):
                ...
                for du_idx in range(DU_PER_LRS):
                    ...
                    du = du_id_map[(pod, rack, layer, du_idx)]
                    ...
                    for l1_idx in l1_targets:

                        # 含义：在同一 (pod, rack, layer) 内，定位编号为 l1_idx 的 L1 节点全局 ID。
                        # 该 ID 与当前 du 组成一条 DU->L1 逻辑连接。
                        l1 = l1_id_map[(pod, rack, layer, l1_idx)]

                        # add_netisim_edge(du, l1, ...)
                        # - du: 当前 layer 内第 du_idx 个 DU 全局 ID
                        # - l1: 当前 layer 内第 l1_idx 个 L1 全局 ID
    ...
    #
    # 验证点：
    # - add_netisim_edge 调用次数 = POD_NUM * RACK_NUM * LRS_LAYERS * DU_PER_LRS
    # - 物理链路总数 = 上式 * 4
        ...
        for layer in range(LRS_LAYERS):
            ...
            for rack in range(RACK_NUM):
                ...
                for du_idx in range(DU_PER_LRS):
                    ...
                    # - hrs: 与该 layer 绑定的 HRS 全局 ID（hrs_ids[layer]）
