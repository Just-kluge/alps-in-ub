...
def host_offset(loc: HostLoc) -> int:
    ...
    in_rack_idx = (loc.rack_r * RACK_COLS) + loc.rack_c
    ...
    for layer in range(LRS_LAYERS):
        ...
        for l1_dst_du_link_idx in (0, 1):
            if use_new_rule:
                ...
                # 找到 dst_row 在 port_index 中的位置 k。
                try:
                    k = port_index.index(dst_row)
                except ValueError:
                    # dst_row 不在 port_index 中，跳过该路径。
                    continue
                
                if layer == 0:
                    # Layer 0: 直连组为 port_index[0..3]，绕行组为 port_index[4..7]。
                    if k < 4:
                        # 目的行在直连组内。
                        if l1_dst_du_link_idx == 0:
                            # 直连：DU -> ToR[dst_row]。
                            mid_tor_row = dst_row
                        else:
                            # 绕行：DU -> ToR[port_index[(k+3)%4]] -> ToR[dst_row]。
                            mid_tor_row = port_index[(k + 3) % 4]
                    else:
                        # 目的行在绕行组，需要经前 4 个 ToR 中转。
                        if l1_dst_du_link_idx == 0:
                            # 经 port_index[k%4] 中转。
                            mid_tor_row = port_index[k % 4]
                        else:
                            # 经 port_index[(k+3)%4] 中转。
                            mid_tor_row = port_index[(k + 3) % 4]
                    ...
                    # Layer 1: 直连组为 port_index[4..7]，绕行组为 port_index[0..3]。
                    if k >= 4:
                        # 目的行在直连组内。
                        if l1_dst_du_link_idx == 0:
                            # 直连：DU -> ToR[dst_row]。
                            ...
                            # 绕行：DU -> ToR[port_index[(k+3)%4 + 4]] -> ToR[dst_row]。
                            mid_tor_row = port_index[(k + 3) % 4 + 4]
                    else:
                        # 目的行在绕行组，需要经后 4 个 ToR 中转。
                        if l1_dst_du_link_idx == 0:
                            # 经 port_index[k+4] 中转。
                            mid_tor_row = port_index[k + 4]
                        else:
                            # 经 port_index[(k+3)%4 + 4] 中转。
                            mid_tor_row = port_index[(k + 3) % 4 + 4]
...
def build_same_pod_diff_rack_paths(
    ...
    for layer in range(LRS_LAYERS):
        ...
        # Layer 0/1且使用偶数DU时，应用新的DU->ToR路由规则。
        use_new_rule = (layer in (0, 1) and True)  # 当前 layer 的目标 DU 由 src_l1_idx 决定，可能是偶数或奇数
        ...
        for dst_du_idx in (2 * dst.rack_c, 2 * dst.rack_c + 1):
            ...
            apply_new_rule = use_new_rule and is_even_du

            # HRS->目的 DU 提供 4 条并行链路索引。
            for hrs_dst_du_link_idx in range(4):
                if apply_new_rule:
                    # Layer 0/1 偶数 DU：根据 hrs_dst_du_link_idx 选择对应的 DU->ToR 子路径。
                    ...
                    if layer == 0:
                        # Layer 0: 直连组为 port_index[4..7]，绕行组为 port_index[0..3]。
                        if k >= 4:
                            # 目的行在直连组内。
                            if hrs_dst_du_link_idx == 0:
                                # 子路径 0：直连。
                                mid_tor_row = dst_row
                            else:
                                # 子路径 1,2,3：经直连组的其他 ToR 中转。
                                mid_tor_row = port_index[(k + hrs_dst_du_link_idx) % 4 + 4]
                        else:
                            # 目的行在绕行组，需经直连组中转。
                            mid_tor_row = port_index[hrs_dst_du_link_idx + 4]
                    
                    elif layer == 1:
                        # Layer 1: 直连组为 port_index[0..3]，绕行组为 port_index[4..7]。
                        if k < 4:
                            # 目的行在直连组内。
                            if hrs_dst_du_link_idx == 0:
                                # 子路径 0：直连。
                                ...
                                # 子路径 1,2,3：经直连组的其他 ToR 中转。
                                mid_tor_row = port_index[(k + hrs_dst_du_link_idx) % 4]
                        else:
                            # 目的行在绕行组，需经直连组中转。
                            mid_tor_row = port_index[hrs_dst_du_link_idx]
...
def main() -> None:
    ...
    parser.add_argument(
        "--case-dir",
        default="/app/ns-3-ub/scratch/tp_UB_Mesh",
