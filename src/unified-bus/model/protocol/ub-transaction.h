...
namespace ns3 {
    ...
    enum class TransactionServiceMode : uint8_t {
        ROI = 0,    // Reliable Ordered by Initiator, 由发送方维护
        ROT = 1,    // Reliable Ordered by Target, 由接收方维护
        ROL = 2,    // Reliable Ordered by Lower Layer, 由下层协议维护
        UNO = 3     // Unreliable No Order, 不保证可靠性和顺序
    };
