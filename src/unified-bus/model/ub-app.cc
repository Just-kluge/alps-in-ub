...
{
    // 每个traffic record即为一个task, ，每个task本身有自己全局唯一地taskid
    // 但我们为每个task创建一个全局唯一的jettynum，来维护这个task的状态，查询这个task的tpns等
    ...
    if (record.opType == "MEM_STORE" || record.opType == "MEM_LOAD") {
        ...
        ubFunc->CreateJetty(record.sourceNode, record.destNode, m_jettyNum); //为每个任务task创建全局唯一地一个jetty，可视为taskid
        ...
        if (bindRst) {
            ...
            SetFinishCallback(MakeCallback(&UbApp::OnTaskCompleted, this), curr_jetty); //这就是我们的qp_finish()回调函数
            ...
            NS_LOG_INFO("[APPLICATION INFO] taskId: " << record.taskId << ",start time:" << Simulator::Now().GetNanoSeconds() << "ns");
