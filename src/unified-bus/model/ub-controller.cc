                            ...
                            uint32_t dstTpn, Ptr<UbCongestionControl> congestionCtrl)
{
    ...
    Ptr<UbTransportChannel> tp = CreateObject<UbTransportChannel>(); // 代表1条流
    ...
    m_numToTp[srcTpn] = tp; // 映射关系
    m_transportsCount++;
    currentNode->GetObject<UbSwitch>()->RegisterTpWithAllocator(tp, sport, priority);  // register TP with allocator,其实这里应该用dport，但host端目前没有dport的概念，且端侧只有1个端口，所以暂时用sport代替
